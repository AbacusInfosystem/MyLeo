...
function Bind_Grid(obj, name,list_name) {
    var table = $("<table>");

    Bind_Header(table, obj);

    Bind_Rows(table, obj, name, list_name);
...
function Bind_Rows(table, obj, name) {
    ...
    for (var i = 0; i < obj.Grid_Detail['Records'].length; i++) {
        ...
        if (obj.Grid_Detail['Identity_Columns'].length > 0) {
            ...
            var radio = $("<input>", { type: "checkbox", value: obj.Grid_Detail['Records'][i]['' + obj.Grid_Detail['Identity_Columns'][0]], name: name }).appendTo(label);
        ...
        for (var j = 0; j < obj.Grid_Detail['Show_Columns'].length; j++) {
            if (j <= obj.Grid_Detail['Show_Columns'].length) {
                ...
                $("<input>", { type: "hidden", value: obj.Grid_Detail['Records'][i]['' + obj.Grid_Detail['Show_Columns'][j]], name: obj.Grid_Detail['Show_Columns'][j] }).appendTo(td);
            }
        }
    }
}

function Bind_Rows(table, obj, name, list_name) {
    var tbody = $("<tbody>").appendTo(table);
